...
public class CPVQueries {
    ...
    public List<Cpv> retrieve(CpvRequestContainer requestedCpvEvent) {
    ...
    public void create(Cpv model) {
        EntityManager entityManager = Database.CONNECTOR.getEntityManager();
    ...
    public Cpv validateCode(String cpvCode) {
        ...
        List<Cpv> list = em.createQuery(criteria).setMaxResults(100).getResultList();
        if (list.size() == 1) {
            return list.get(0);
        } else {
