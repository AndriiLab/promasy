...
public class CPVRepository {
    ...
    public List<Cpv> get(String requestedCpv, int depth) {
        list.clear();
        ...
        if (requestedCpv.isEmpty()) {
            ...
            CPVRequestObject reqObj = prepareDigitCpv(requestedCpv, depth);
    ...
    public Cpv validateCode(String cpvCode) {
        ...
        List<Cpv> results = entityManager.createQuery(criteria).setMaxResults(100).getResultList();
        if (results.size() == 1) {
